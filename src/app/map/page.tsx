...
export default function MapPage() {
  ...
  const categoryStyles: Record<MarkerCategory, { icon: keyof typeof LucideIcons; color: string }> = {
    hotel: { icon: "Hotel", color: "#3B82F6" },        // ฟ้า
    restaurant: { icon: "UtensilsCrossed", color: "#EF4444" }, // แดง
    attraction: { icon: "MapPin", color: "#10B981" },  // เขียว
  };
  ...
  const renderIconToString = (IconComp: ComponentType<any>, color: string, size = 22) =>
    renderToString(<IconComp size={size} color={color} strokeWidth={2} />);
  ...
  useEffect(() => {
    ...
    systemPoints.forEach((m) => {
      ...
      const IconComp = (LucideIcons as any)[icon] as ComponentType<any>;

      // ตัว marker (DOM) ใส่ SVG icon ลงไป
      const markerEl = document.createElement("div");
      markerEl.innerHTML = renderIconToString(IconComp, color, 22);
  ...
  return (
    <div className="w-full h-screen relative overflow-hidden font-sans">
      ...
      {locationInfo && (
        <div
          className={`absolute left-1/2 -translate-x-1/2 translate-y-1/3 bottom-20 w-[90%] max-w-sm 
