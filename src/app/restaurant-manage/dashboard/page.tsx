...
import StatCard from '@/components/restaurant-manage/StatCard';
import ChartCard from '@/components/restaurant-manage/ChartCard';
...
const ReviewProgressBar = ({ label, value, percentage }) => (
  <div className="flex items-center gap-3 w-full">
    ...
    <span className="w-8 text-right text-gray-700 font-semibold text-sm">{value.toFixed(1)}</span>
  </div>
);
    ...
    <div className="lg:col-span-2 p-6 pb-10 bg-white rounded-xl border border-neutral-200 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6 items-center">
        ...
        <div className="md:col-span-3 flex flex-col justify-center gap-4">
          <ReviewProgressBar label="Quality" value={9.2} percentage={92} />
          <ReviewProgressBar label="Service" value={9.8} percentage={98} />
          <ReviewProgressBar label="Price" value={9.0} percentage={90} />
          <ReviewProgressBar label="Cleanliness" value={9.5} percentage={95} />
        </div>
      </div>
    </div>
);
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 font-['Manrope']">
      ...
      <div className="flex flex-1 overflow-hidden">
        <SideNav />
        {/* main ไม่ต้องเป็น flex-col แล้ว */}
        <main className="flex-1 p-7 overflow-y-auto">
          ...
          {/* --- ✅ [แก้ไข] เปลี่ยน flex-1 เป็น h-96 --- */}
          <div className="h-115">
            <ChartCard 
